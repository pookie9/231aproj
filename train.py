...
def tint_image(img,amount=None):
    if amount==None:
        amount=np.array([100,0,100])
    return img+amount

def noise(img,amount=None):
    if amount==None:
        amount=50
    noise=np.random.random_integers(-amount,amount,size=img.shape)
    return img+noise

def rotate(img, amount=20.0):
    import scipy
    return scipy.misc.imrotate(img,amount)

def blur(img, amount=None):
    import cv2
    return cv2.filter2D(img,-1,np.ones((5,5),np.float32)/25)

def distort(img, amount=None):
    import cv2
    fx=5
    fy=5
    cx=10
    ...
    print img.shape
    print type(img)
    return cv2.fisheye.distortPoints(img,K,D=D)
